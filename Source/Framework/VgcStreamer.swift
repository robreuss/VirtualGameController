...
class VgcStreamer: NSObject, NetServiceDelegate, StreamDelegate {
    ...
    @objc func delayedWriteData(_ timer: Timer) {
        ...
        vgcLogDebug("Timer triggered to process data send queue (\(dataSendQueue.count) messages) to stream \(outputStream) [\(Date().timeIntervalSince1970)]")
        writeData(Data(), toStream: outputStream)
    }
    ...
    func writeData(_ data: Data, toStream: OutputStream) {
        ...
        if (VgcManager.appRole == .Peripheral) && VgcManager.peripheral.haveOpenStreamsToCentral == false {
            vgcLogDebug("No connection so clearing write queue (\(dataSendQueue.count) messages)")
            dataSendQueue = [Data]()
            return
        }
        ...
        if !toStream.hasSpaceAvailable {
            ...
            if data.count > 0 {
                ...
                vgcLogDebug("Appended data queue (\(dataSendQueue.count) total messages)")
            }
        ...
        if dataSendQueue.count > 0 {
            vgcLogDebug("Processing data queue (\(dataSendQueue.count) messages)")
    ...
    func stream(_ aStream: Stream, handle eventCode: Stream.Event) {
        ...
        case Stream.Event.hasBytesAvailable:
            ...
            while dataBuffer.length > 0 {
                ...
                let headerIdentifier = dataBuffer.subdata(with: NSRange(location: 0, length: 4))
                if headerIdentifier == headerIdentifierAsNSData as Data {
                    var elementIdentifierUInt8: UInt8 = 0
                    let elementIdentifierNSData = dataBuffer.subdata(with: NSRange(location: 4, length: 1))
                    ...
                    let valueLengthNSData = dataBuffer.subdata(with: NSRange(location: 5, length: 4))
                    ...
                    if VgcManager.netServiceLatencyLogging {
                        ...
                        let timestampNSData = dataBuffer.subdata(with: NSRange(location: 9, length: 8))
                ...
                elementValueData = dataBuffer.subdata(with: NSRange(location: headerLength, length: expectedLength))
                let dataRemainingAfterCurrentElement = dataBuffer.subdata(with: NSRange(location: headerLength + expectedLength, length: dataBuffer.length - expectedLength - headerLength))
