...
class VgcBrowser: NSObject, NetServiceDelegate, NetServiceBrowserDelegate, StreamDelegate, VgcStreamerDelegate {
    ...
    init(peripheral: Peripheral) {
        ...
        streamer[.largeData] = VgcStreamer(delegate: self, delegateName: "Browser")
        streamer[.smallData] = VgcStreamer(delegate: self, delegateName: "Browser")
        
        vgcLogDebug("Setting up NSNetService for browsing")
        
        localService = NetService(domain: VgcManager.serviceDomain, type: VgcManager.bonjourTypeCentral, name: deviceName, port: 0)
    ...
    func receivedNetServiceMessage(_ elementIdentifier: Int, elementValue: Data) {
        ...
        case .playerIndex:
            ...
            if deviceIsTypeOfBridge() {
                peripheral.controller.playerIndex = GCControllerPlayerIndex(rawValue: playerIndex)!
            }
    ...
    func sendDeviceInfoElement(_ element: Element!) {
        ...
        if VgcManager.appRole == .Peripheral || VgcManager.appRole == .MultiplayerPeer {
            outputStreamLarge = outputStream[.largeData]
            outputStreamSmall = outputStream[.smallData]
