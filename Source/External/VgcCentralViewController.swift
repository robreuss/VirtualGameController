...
@objc public class VgcCentralViewController: UIViewController {
    ...
    public override func viewDidLoad() {
        ...
        self.imageView.gestureRecognizers = [gr]
        
        // Make debug view width suitable for a given device type
        if UIDevice.current.userInterfaceIdiom == .phone {
            self.debugViewWidth = self.scrollview.bounds.size.width * 0.80
        } else if UIDevice.current.userInterfaceIdiom == .pad {
            self.debugViewWidth = self.scrollview.bounds.size.width * 0.40
            if self.debugViewWidth > 500 { self.debugViewWidth = 300 }
        } else {
            self.debugViewWidth = self.scrollview.bounds.size.width * 0.17
        }
        
        #if !os(tvOS)
            // Hidden text field to receive iCade controller input
            self.iCadeTextField = UITextField(frame: CGRect(x: -1, y: -1, width: 1, height: 1))
            self.iCadeTextField.addTarget(self, action: #selector(self.receivedIcadeInput(sender:)), for: .editingChanged)
            //iCadeTextField.autocorrectionType = .No
            self.view.addSubview(self.iCadeTextField)
        #endif
        ...
        // Used to determine if an external keyboard (an iCade controller) is paired - DEPRACATED
        /*
        #if !os(tvOS)
        NotificationCenter.default.addObserver(self, selector: #selector(self.keyboardWillShow(notification:)), name: NSNotification.Name.UIKeyboardWillShowNotification, object: nil)
        ...
        */
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(VgcCentralViewController.gotPlayerIndex), name: NSNotification.Name(rawValue: VgcNewPlayerIndexNotification), object: nil)
            ...
            if VgcManager.iCadeControllerMode != .Disabled { self.iCadeTextField.becomeFirstResponder() }
        #endif
    }
    ...
    func testForPairedExternalKeyboard(aNotification: NSNotification) {
        ...
        if let keyboardFrame = aNotification.userInfo?[NSNotification.Name("UIKeyboardFrameBeginUserInfoKey")] as? CGRect {
            if self.isExternalKeyboardFrame(keyboardFrame: keyboardFrame) {
                ...
                self.iCadeTextField.resignFirstResponder()
            }
        } else {
    ...
    @objc func keyboardWillShow(notification: NSNotification) {
        self.testForPairedExternalKeyboard(aNotification: notification)
    }
    ...
    @objc func receivedIcadeInput(sender: AnyObject) {
        if VgcManager.iCadeControllerMode != .Disabled && VgcController.iCadeController != nil {
            vgcLogDebug("Sending iCade character: \(String(describing: self.iCadeTextField.text)) using iCade mode: \(VgcManager.iCadeControllerMode.description)")
            var element: Element?
            var value: Int
...
*/
        ...
        self.elementDebugViewLookup[newController] = elementDebugView
        ...
        newController.extendedGamepad?.valueChangedHandler = { (_: GCExtendedGamepad, _: GCControllerElement) in
            
            self.refreshDebugViewForController(controller: newController)
        }
        
        newController.extendedGamepad?.leftThumbstick.xAxis.valueChangedHandler = { _, _ in
            
            //print("HANDLER: Left thumbstick xAxis: \(value)")
        }
        
        newController.extendedGamepad?.buttonA.valueChangedHandler = { _, value, pressed in
            
            print("[SAMPLE] HANDLER:Button A value: \(pressed), \(value)")
        }
        
        newController.extendedGamepad?.leftThumbstick.valueChangedHandler = { _, _, _ in
            
            //print("HANDLER: Left thumbstick: \(xValue), \(yValue)")
        }
        
        // Refresh on all gamepad changes (Global handler)
        newController.gamepad?.valueChangedHandler = { (_: GCGamepad, _: GCControllerElement) in
            ...
            newController.microGamepad?.valueChangedHandler = { (_: GCMicroGamepad, _: GCControllerElement) in
        ...
        newController.motion?.valueChangedHandler = { (_: VgcMotion) in
        ...
        Elements.customElements.valueChangedHandler = { controller, _ in
        ...
        newController.elements.custom[CustomElementType.Keyboard.rawValue]!.valueChangedHandler = { controller, _ in
        ...
        newController.elements.custom[CustomElementType.FiddlestickX.rawValue]!.valueChangedHandler = { controller, _ in
            
            self.refreshDebugViewForController(controller: controller)
        }
        
        // Demonstrates receiving a message as a Bridge from a Central
        VgcManager.elements.rightTrigger.valueChangedHandlerForPeripheral = { _ in
