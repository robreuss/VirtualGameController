...
+ (NSInteger)netServiceHeaderLength;
+ (void)setNetServiceHeaderLength:(NSInteger)value;
+ (uint32_t)headerIdentifier;
+ (void)setHeaderIdentifier:(uint32_t)value;
...
- (void)setupFilterConstant;
- (void)start;
- (void)stop;
- (void)sendElementState:(Element * __nonnull)element;
- (double)Norm:(double)x y:(double)y z:(double)z;
- (double)Clamp:(double)v min:(double)min max:(double)max;
@property (nonatomic, readonly) double kAccelerometerMinStep;
@property (nonatomic, readonly) double kAccelerometerNoiseAttenuation;
- (nonnull instancetype)init OBJC_DESIGNATED_INITIALIZER;
@end
